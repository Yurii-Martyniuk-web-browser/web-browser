...
public class HtmlParser {
    ...
    public Document parse(String html) {
        ...
        while (pos < html.length()) {
            ...
            if (lt == -1) {
                ...
                if (!text.isBlank()) stack.peek().addChild(new TextNode(text));
                break;
            }
    ...
    private void handleTag(String content, Deque<Element> stack) {
        ...
        if (isClosing) {
            if (!stack.isEmpty()) {
                if (stack.peek().tagName().equals(tagName)) {
                    stack.pop();
                }
            }
        } else {
    ...
    private void parseAttributes(Element el, String rawAttrs) {
        java.util.regex.Matcher m = java.util.regex.Pattern.compile("(\\w+)=\"([^\"]*)\"").matcher(rawAttrs);
        while (m.find()) {
